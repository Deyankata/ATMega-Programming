...
uint16_t getAdcValue() {
	...
	for (int i = 0; i < 16; i++){
		...
		// Store the value as temp and add to sum
		temp = ADCL | ((uint16_t)ADCH << 8);
		sum += temp;

	}
	
	lastCaptured = sum / 16; // take the average of the sum
...
void storeVoltage(void){
	if(bufferSize == 0){	
		bufferStart = malloc(100 * sizeof(uint16_t)); // Allocate memory for 100 entries; 
		bufferSize = 50 * sizeof(bufferStart);
	}
