...
// A variable used in the ISR to temporarily hold the value of the current process
ProcessID tempProcess;
...
ISR(TIMER2_COMPA_vect) {
	...
	if ((os_getInput() & (1 | (1 << 3))) == (1 | (1 << 3))){ // if Enter + ESC (Task manager)
	 ...
	 // Store the current PID temporarily
	 tempProcess = currentProc;
	...
	if (os_processes[tempProcess].state == OS_PS_BLOCKED){
		os_processes[tempProcess].state = OS_PS_READY;
	}
	
	if (os_getStackChecksum(currentProc) != os_processes[currentProc].checksum){
			// The stack has changed, issue an error message
			os_error("Stack Checksum");
 	}
...
void os_yield(void){
	...
	// Save the current state of the critical section locally
	uint8_t curCriticalSections = criticalSectionCount;
	uint8_t globalInterruptBit = SREG >> 7;
	
	// Leave all critical sections
	criticalSectionCount = 1;
	os_leaveCriticalSection();
	...
	// Restore state critical sections
	criticalSectionCount = curCriticalSections;
	SREG &= globalInterruptBit << 7;
