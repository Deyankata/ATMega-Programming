...
ProcessID os_exec(Program *program, Priority priority) {
	...
	*stack_ptr.as_ptr = (uint8_t)(((uint16_t)program) & 0xFF); // Low byte
	stack_ptr.as_ptr--;
	*stack_ptr.as_ptr = (uint8_t)(((uint16_t)program >> 8) & 0xFF); // High byte
...
void os_initScheduler(void){
	...
	struct program_linked_list_node *current = autostart_head;
	while (current != NULL){
		if (os_isRunnable(os_getProcessSlot(idlePID))){ // if the idle process is runnable
			os_exec(current->program, DEFAULT_PRIORITY);
		}
