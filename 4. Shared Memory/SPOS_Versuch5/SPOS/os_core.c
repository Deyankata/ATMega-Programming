...
//#define ProgramData 215
//uint16_t SRAMBeginAddress = 0x100;
...
void os_init(void) {
    ...
    if ((uint16_t)(&__heap_start) >= INTERNAL_SRAM_START + HEAPOFFSET){
	    os_error("Error: Safety Distance");
    }
...
void os_errorPStr(const char *str) {
	uint8_t temp = SREG >> 7;
	cli(); // disable global interrupts (SREG &= ~(1 << SREG_I))
